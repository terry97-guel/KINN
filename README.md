...
Training configs can be found in `kinn/configs` and, use arguments to choose model and dataset.


```
python3 kinn/main.py --configs "${MODEL}/${DATASET}.py"
```
Choose, 
`${MODEL}` from `[FC_PRIMNET, PRIMNET, PCC_PRIMNET, PRIMNET_FULL]`
`${DATASET}` from `[FINGER, ABAQUS_32, ELASTICA]`
(e.g. To train `PureNN` on `FINGER` dataset,  `python3 kinn/main.py --configs "PRIMNET/FINGER.py"`)


## Train (with Discrete Optimization)
```
python3 kinn/main_kas.py --configs "${MODEL}/${DATASET}.py" --generation_num 100 --agent_num 50 --epochs 1000
```
